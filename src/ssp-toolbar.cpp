...
SspToolbarManager::~SspToolbarManager()
{
	...
	m_browserDocks
		.clear(); // Pointers in m_browserDocks are owned by m_mainWindow
					  ...
					  const QString &ip)
{
	...
	if (!m_sourceActions.contains(sourceKey)) {
		blog(LOG_ERROR, "add source action in tool bar %s",
		     sourceKey.toStdString().c_str());
		QAction *action = new QAction(
			sourceName,
			this); // Parented to this, will be deleted with this
			...
			[this, sourceName, ip, sourceKey](bool checked) {
				...
				if (checked != dockVisible) {
					if (checked) {
						if (!dockExists ||
						    !m_browserDocks
							    [sourceKey]) { // Check if dock is null also
							showBrowserDock(
								sourceName, ip,
								sourceKey);
...
{
	QString sourceKey = QString("%1_%2").arg(sourceName).arg(ip);
	blog(LOG_ERROR, "remove source action in tool bar %s",
	     sourceKey.toStdString().c_str());
	if (m_sourceActions.contains(sourceKey)) {
	...
	if (m_browserDocks.contains(sourceKey)) {
		QDockWidget *dock = m_browserDocks.take(
			sourceKey); // Use take to remove and get value
		if (dock != nullptr) {
...
void SspToolbarManager::suppressUnloadDialog(QCefWidget *widget)
{
	if (widget) {
		blog(LOG_INFO,
		     "Attempting to suppress unload dialog via JavaScript.");
	...
	QTimer::singleShot(1000, this, [this, sourceKey]() {
		...
		if (dock && dock->widget()) {
			...
			if (browser) {
				...
				blog(LOG_WARNING,
				     "No QCefWidget found in dock for key %s.",
				     sourceKey.toStdString().c_str());
			}
		} else {
			blog(LOG_WARNING,
			     "Dock or its widget is null for key %s.",
			     sourceKey.toStdString().c_str());
		}
	});
}
						  ...
						  const QString &sourceKey)
{
	...
	dock->setObjectName("sspDock_" +
			    sourceKey); // Set object name to retrieve key later
...
void SspToolbarManager::shutdown()
{
	...
	if (s_instance->thread() == nullptr ||
	    !s_instance->thread()->isRunning()) {
...
void SspToolbarManager::shutdownInternal()
{
	...
	blog(LOG_INFO,
	     "[SSPToolbar] shutdownInternal: Browser docks processed and cleared.");
	...
	blog(LOG_INFO,
	     "[SSPToolbar] shutdownInternal: Finished cleanup (self scheduled for deletion). s_instance is now %s.",
	     s_instance ? "NOT NULL" : "NULL");
}
