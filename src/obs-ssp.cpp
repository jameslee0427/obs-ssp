...
bool obs_module_load(void)
{
	...
	if (!check_obs_version_compatibility()) {
		...
		ssp_blog(LOG_ERROR, "Incompatible OBS Studio version. This plugin requires OBS Studio %d.%d.%d or higher.",
			MIN_OBS_VERSION_MAJOR, MIN_OBS_VERSION_MINOR, MIN_OBS_VERSION_PATCH);
		return false;
	}
	...
	if (!check_browser_module_available()) {
		...
		QMetaObject::invokeMethod(main_window, [main_window]() {
			QMessageBox::warning(
				main_window,
				obs_module_text("SSPPlugin.BrowserCheck.Title"),
				obs_module_text("SSPPlugin.BrowserCheck.Error"),
				QMessageBox::Ok);
		}, Qt::QueuedConnection);
		
		ssp_blog(LOG_WARNING, "OBS Browser module not found! Some features may not work correctly.");
		ssp_blog(LOG_WARNING, "Please upgrade to a newer version of OBS that includes the Browser module.");
	}
...
static bool check_obs_version_compatibility()
{
	...
	ssp_blog(LOG_INFO, "OBS Studio version: %d.%d.%d", 
		obs_major, obs_minor, obs_patch);
...
void obs_module_unload()
{
	   ...
	   ssp_blog(LOG_INFO, "[obs-ssp] obs_module_unload: Called.");
        ...
        ssp_blog(LOG_INFO, "[obs-ssp] obs_module_unload: SspToolbarManager::shutdown() returned. s_instance is %s.", SspToolbarManager::checkInstance() ? "NOT NULL" : "NULL (Note: s_instance is static in SspToolbarManager, check its value there)");


        ssp_blog(LOG_INFO, "[obs-ssp] obs_module_unload: Cleaning up CameraStatusManager...");
        CameraStatusManager::instance()->cleanup();
        CameraStatusManager::destroyInstance();
        ssp_blog(LOG_INFO, "[obs-ssp] obs_module_unload: CameraStatusManager cleaned up.");

        ssp_blog(LOG_INFO, "[obs-ssp] obs_module_unload: Goodbye!"); // Changed from ssp_blog for consistency example
}
